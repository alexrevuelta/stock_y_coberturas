...
def get_odoo_data_for_brand(marca, config):
    ...
    try:
        ...
        return report[:10] # Devolvemos los 10 más vendidos de esta marca
...
def generate_brand_html(marca, data):
    ...
    for item in data:
        clase = "color: #5cb85c;" # Verde (Stock OK)
        if item['cobertura'] < 7: clase = "color: #d9534f; font-weight: bold;" # Rojo (Crítico)
        elif item['cobertura'] < 15: clase = "color: #f0ad4e; font-weight: bold;" # Naranja (Bajo)
...
if __name__ == "__main__":
    if not os.path.exists("config.json"):
        ...
        full_email_body = f"""<html><body style="font-family: sans-serif; padding: 20px;">
            <h1 style="color: #333; text-align: center; border-bottom: 2px solid #333; padding-bottom: 10px;">📈 Reporte de Stock y Cobertura</h1>
            <p style="text-align: center; color: #666;">Informe generado el {datetime.now().strftime('%d/%m/%Y a las %H:%M')}</p>
        """
        ...
        for marca in config.get("marcas", []):
            ...
            if data_marca:
                ...
                log(f"⚠️ No se encontraron productos para {marca}.")

        full_email_body += "</body></html>"

        if any_data:
            msg = MIMEMultipart()
            msg['Subject'] = f"📊 REPORTE STOCK POR MARCAS - {datetime.now().strftime('%d/%m/%Y')}"
